...
pub fn ret_val_demotion(
    ...
    let ret_blocks = function
        ...
        .filter(|block| {
            block
                ...
                .map_or(false, |term| matches!(term, Instruction::Ret(..)))
        })
        .collect::<Vec<_>>();

    // Each `ret` must be returning a `load`, so that we can take the pointer argument to the
    // `load` and return that instead.
    let Some(blocks_and_ptrs) = ret_blocks
        .iter()
        .map(|block| {
            block.get_terminator(context).and_then(|term| {
                if let Instruction::Ret(ret_val, _ty) = term {
                    if let Some(Instruction::Load(ptr_val)) = ret_val.get_instruction(context) {
                        // Pair the block with the pointer argument value.
                        Some((*block, *ptr_val))
                    } else {
                        // Not a `load`.
                        None
                    ...
                    unreachable!("Block definitely has a ret.")
                }
            })
        })
    ...
    let blocks_and_ptrs = rustc_hash::FxHashMap::from_iter(blocks_and_ptrs);
    ...
    // And it now takes an extra argument pointing to the destination for the return value.
    ...
    // Update each `ret` to copy the return value to the 'out' arg and then return the pointer.
    for ret_block in ret_blocks {
        // A special case where we're replacing the terminator.  We can just pop it off the end of
        // the block and add new instructions.
        ...
        let ptr_val = blocks_and_ptrs.get(&ret_block).unwrap();
        ret_block
            .ins(context)
            .mem_copy_val(ptr_arg_val, *ptr_val)
            .add_metadatum(context, md_idx);
        ret_block
            .ins(context)
            .ret(*ptr_val, ptr_ret_type)
            .add_metadatum(context, md_idx);
    }
    ...
    for (calling_func, calling_block, call_val) in call_sites {
        ...
        // Replace the call with the new `get_local` then insert the `call` and the `load` after it.
