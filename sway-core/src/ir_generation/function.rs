...
impl<'eng> FnCompiler<'eng> {
        ...
        match &ast_node.content {
            ty::TyAstNodeContent::Declaration(td) => match td {
                ...
                ty::TyDeclaration::EnumDeclaration(decl_id) => {
                    ...
                    create_enum_type(self.type_engine, context, &ted.variants).map(|_| ())?;
                    Ok(None)
                }
                ...
                ty::TyDeclaration::FunctionDeclaration(_) => unexpected_decl("function"),
                ty::TyDeclaration::TraitDeclaration(_) => unexpected_decl("trait"),
                ty::TyDeclaration::StructDeclaration(_) => unexpected_decl("struct"),
                ty::TyDeclaration::AbiDeclaration(_) => unexpected_decl("abi"),
                ty::TyDeclaration::GenericTypeForFunctionScope { .. } => {
                    unexpected_decl("generic type")
                }
                ty::TyDeclaration::ErrorRecovery { .. } => unexpected_decl("error recovery"),
                ty::TyDeclaration::StorageDeclaration(_) => unexpected_decl("storage"),
            },
    ...
    fn compile_contract_call(
        ...
        let user_args_val = match compiled_args.len() {
            ...
            1 => {
                    ...
                    .is_copy_type()
                {
                    ...
                    // the unnecessary copying eventually.
                    ...
                    self.current_block.ins(context).mem_copy_val(arg0_var, arg0);

                    // NOTE: Here we're casting the original local variable to an integer.
                    let int_ty = Type::new_uint(context, 64);
                    self.current_block.ins(context).ptr_to_int(arg0_var, int_ty)
                }
            }
            _ => {
                ...
                self.current_block
                    .ins(context)
                    .ptr_to_int(user_args_struct_val, int_ty)
                    .add_metadatum(context, span_md_idx)
            }
        };
        ...
        let gep_val =
            self.current_block
                .ins(context)
                .get_elem_ptr_with_idx(ra_struct_val, b256_ty, 0);
        self.current_block
            .ins(context)
            .store(gep_val, addr)
            .add_metadatum(context, span_md_idx);
        ...
        let gep_val =
            self.current_block
                .ins(context)
                .get_elem_ptr_with_idx(ra_struct_val, u64_ty, 1);
        self.current_block
            .ins(context)
        ...
        let gep_val =
            self.current_block
                .ins(context)
                .get_elem_ptr_with_idx(ra_struct_val, u64_ty, 2);
        self.current_block
            .ins(context)
    ...
    fn compile_unsafe_downcast(
        ...
        let gep_val = self.current_block.ins(context).get_elem_ptr_with_idcs(
            compiled_value,
            variant_type,
        ...
        // Load the value.
        Ok(self.current_block.ins(context).load(gep_val))
    }
    ...
    fn compile_enum_tag(
        ...
        let struct_val = self.compile_expression(context, md_mgr, &exp)?;
    ...
    fn compile_var_expr(
        ...
        if let Some(var) = self.get_function_var(context, name) {
            ...
            if val.get_type(context).map_or(false, |ty| {
                ty.is_ptr(context)
                    && !ty
                        .get_inner_type(context)
                        .map(|inner_ty| inner_ty.is_aggregate(context))
                        .unwrap_or(false)
            }) {
                Ok(self
                    .current_block
    ...
    fn compile_reassignment(
        ...
        // Create a GEP by following the chain of LHS indices.
        let gep_indices = ast_reassignment
            .lhs_indices
            .iter()
            .scan(
                ast_reassignment.lhs_type,
                |cur_type_id, idx_kind| match idx_kind {
                    ProjectionKind::StructField { name: idx_name } => {
                        ...
                        if let TypeInfo::Struct {
                            ...
                            ..
                        } = self.type_engine.get(*cur_type_id)
                        {
                                ...
                                struct_fields
                                    ...
                                    .map(|(field_idx, field_type_id)| {
                                        *cur_type_id = field_type_id;
                                        ...
                                        Constant::get_uint(context, 64, field_idx as u64)
                                    }),
                            )
                        } else {
                            Some(Err(CompileError::Internal(
                                "Unknown struct in reassignment.",
                                ast_reassignment.lhs_base_name.span(),
                            )))
                        }
                    }
                    ProjectionKind::TupleField { index, .. } => {
                        Some(Ok(Constant::get_uint(context, 64, *index as u64)))
                    }
                    ProjectionKind::ArrayIndex { index, .. } => {
                        Some(self.compile_expression(context, md_mgr, index))
                    }
                },
            )
    ...
    fn compile_struct_field_expr(
        ...
        let TypeInfo::Struct {
            ...
            fields: struct_fields,
            ..
        } = self.type_engine.get(struct_type_id) else {
            return Err(CompileError::Internal(
        ...
        let field_type =
            convert_resolved_typeid(self.type_engine, context, &field_type_id, &ast_field.span)?;
        let gep_val = self
            .current_block
            .ins(context)
    ...
    fn compile_enum_expr(
        ...
        let enum_type = create_enum_type(self.type_engine, context, &enum_decl.variants)?;
        let tag_value =
        ...
        let gep_val = self
            .current_block
            .ins(context)
            .get_elem_ptr_with_idx(enum_val, u64_ty, 0)
            .add_metadatum(context, span_md_idx);
        self.current_block
            .ins(context)
            .store(gep_val, tag_value)
            .add_metadatum(context, span_md_idx);
        ...
        if field_tys.len() == 1 || contents.is_none() {
            Ok(enum_val)
        } else {
            ...
            let gep_val = self
                .current_block
                .ins(context)
                .get_elem_ptr_with_idx(enum_val, contents_type, 1)
                .add_metadatum(context, span_md_idx);
            Ok(self
                .current_block
                .ins(context)
    ...
    fn compile_tuple_elem_expr(
        ...
        let tuple_type = convert_resolved_typeid(self.type_engine, context, &tuple_type, &span)?;
        tuple_type
            ...
            ty if ty.is_struct(context) => {
                ...
                for (field_idx, field_type) in fields.into_iter().enumerate() {
                    ...
                    let gep_val = self
                        .current_block
                        .ins(context)
                        .get_elem_ptr_with_idx(rhs, field_type, field_idx)
                        .add_metadatum(context, span_md_idx);
                    let rhs = self
                        .current_block
                        .ins(context)
    ...
    fn compile_union_or_string_storage_read(
        ...
        if number_of_elements > 0 {
            ...
            let indexed_value_val_b256 = self
                .current_block
                .ins(context)
                .cast_ptr(value_val_b256, b256_ty)
                .add_metadatum(context, span_md_idx);
    ...
    fn compile_union_or_string_storage_write(
        ...
        if number_of_elements > 0 {
            ...
            let indexed_value_ptr_val_b256 = self
                .current_block
                .ins(context)
                .cast_ptr(value_ptr_val_b256, b256_ty)
                .add_metadatum(context, span_md_idx);
