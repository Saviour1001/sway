...
impl ty::TyExpression {
    ...
    pub(crate) fn type_check(mut ctx: TypeCheckContext, expr: Expression) -> CompileResult<Self> {
        ...
        append!(
            ctx.unify(typed_expression.return_type, &expr_span),
            warnings,
            errors
        );
    ...
    fn type_check_code_block(
        ...
        append!(ctx.unify(block_return_type, &span), warnings, errors);
    ...
    fn type_check_delineated_path(
        ...
        let maybe_function = {
            ...
            TypeBinding::type_check_with_ident(&mut call_path_binding, ctx.by_ref())
                ...
                .map(|(func_decl, function_decl_id, function_type_subst_list)| {
                    (
                        func_decl,
                        function_decl_id,
                        function_type_subst_list,
        ...
        let maybe_enum = {
            ...
            TypeBinding::type_check_with_ident(&mut call_path_binding, ctx.by_ref())
                .flat_map(|unknown_decl| unknown_decl.expect_enum(decl_engine, &span))
        ...
        let exp = match (is_module, maybe_function, maybe_enum, maybe_const) {
            ...
            (false, Some(maybe_function), None, None) => {
                let (function_decl, function_decl_id, function_type_subst_list, call_path_binding) =
                    maybe_function;
    ...
    fn type_check_array(
        ...
        for typed_elem in &typed_contents[1..] {
            let (mut new_warnings, mut new_errors) = ctx
                ...
                .unify(typed_elem.return_type, &typed_elem.span);
